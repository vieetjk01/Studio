...
// ─── Copy ảnh đã lọc sang thư mục Drive (công cụ Lọc ảnh) ─────────────────────

export type DriveCopyOutcome =
  | {
      ok: true;
      folderId: string;
      folderUrl: string;
...
/**
 * CHÉP THẲNG trên Drive (files.copy) các file ảnh (theo fileId) sang MỘT thư mục
 * trên Drive của studio — KHÔNG tải về máy, KHÔNG qua băng thông server. Dùng
 * OAuth studio (scope drive.file) nên chỉ chép được file & thư mục do ỨNG DỤNG
 * tạo/mở (vd "JPG Goc" của hợp đồng đồng bộ qua desktop). Link Drive công khai
 * dán tay (không do app tạo) sẽ báo lỗi permission.
 *
 * - `targetFolderId`: chép vào thư mục có sẵn (studio tự chọn).
 * - hoặc `newFolder {name, parentId}`: tạo thư mục mới (thường ngay trong thư mục
 *   gốc) rồi chép vào.
 */
export async function copyFilesToStudioDrive(
  ownerId: string,
  opts: {
...
): Promise<DriveCopyOutcome> {
  const row = await loadStudioDrive(ownerId);
  if (!row?.refresh_token) return { ok: false, error: "not_connected" };
  ...
  // Xác định thư mục đích: dùng thư mục sẵn có, hoặc tạo mới trong thư mục gốc.
  let folderId = opts.targetFolderId?.trim() || "";
  let folderName = "";
  if (folderId) {
    try {
  ...
  // Chép từng file (giới hạn song song để không vượt quota Drive).
